...
def tako_run():
    ...
    while 1:
        try:
            ...
            # if len(buff) > 32:
            #    buff[-32:] # take last 32 chars only
            pack = buff
            while pack:
                pack, buff = packet_fnd(buff)
            if not qmsg.empty():
...
@route_tako.put("/write", tags=["tags"], summary="write tags")
...
if __name__ == '__main__':        # Run from command line
    ...
    parser.add_argument('--debug', '-d', default=False, action='store_true', help='debug rx')
